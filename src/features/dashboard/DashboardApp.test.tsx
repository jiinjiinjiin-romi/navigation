...
describe('DashboardApp', () => {
  test('guards dashboard pages behind mock login and opens overview after login', () => {
    ...
    expect(screen.getByTestId('dashboard-app')).toHaveClass('h-full')
    expect(screen.getByTestId('dashboard-app')).not.toHaveClass('min-h-dvh')
    expect(screen.getByTestId('dashboard-app')).toHaveClass('lg:p-4')
    expect(screen.getByTestId('dashboard-shell')).toHaveClass('bg-white')
    expect(screen.getByTestId('dashboard-shell')).toHaveClass('h-full')
    expect(screen.getByTestId('dashboard-shell')).not.toHaveClass('min-h-dvh')
    expect(screen.getByTestId('dashboard-shell')).toHaveClass('lg:overflow-hidden')
    expect(screen.queryByRole('button', { name: '모바일 메뉴 열기' })).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: '모바일 메뉴 닫기' })).not.toBeInTheDocument()
    expect(screen.getByTestId('dashboard-mobile-nav')).toHaveClass('left-[calc(var(--roadie-dashboard-safe-left,0px)+0.75rem)]')
    expect(screen.getByTestId('dashboard-mobile-nav')).toHaveClass('right-3')
    expect(screen.getByTestId('dashboard-mobile-nav')).not.toHaveClass('inset-x-3')
