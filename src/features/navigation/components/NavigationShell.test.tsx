...
describe('NavigationShell', () => {
  ...
  it('resets the completed demo destination to Sejong before entering manual navigation with a profile', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '다음' }))
    fireEvent.click(await screen.findByTestId('demo-entry-manual-control-button'))
    expect(await screen.findByTestId('navigation-profile-setup')).toBeInTheDocument()
    expect(screen.getByTestId('navigation-profile-setup-content')).toHaveClass('roadie-navigation-density-surface')
  ...
  it('loads backend profiles before entering navigation and selects one', async () => {
    ...
    expect(screen.getByTestId('demo-scenario-card-agent_personality_voice_change').closest('.roadie-navigation-scenario-density')).not.toBeNull()
    expect(screen.getByTestId('demo-scenario-card-agent_personality_voice_change').querySelector(':scope > span')).toHaveClass(
      'inset-x-0',
      'top-0',
      'h-1',
    )
    expect(screen.getByTestId('demo-scenario-card-drowsy_driver')).toHaveClass('min-h-[10.5rem]')
    ...
    expect(within(screen.getByTestId('demo-scenario-card-drowsy_driver')).getByText('01')).toHaveClass('text-xl')
  ...
  it('renders the root risk and demo-ready status in the left extension rail', () => {
    ...
    expect(sideRail).toHaveClass('roadie-navigation-density-surface')
    expect(sideRail).toHaveClass('w-[var(--roadie-nav-side-rail-width)]')
  ...
  it('starts a fixed demo scenario after profile selection', async () => {
    ...
    expect(presenterPanel).toHaveClass('roadie-navigation-density-surface')
    expect(presenterPanel).toHaveClass('w-[var(--roadie-nav-side-rail-width)]')
  ...
  it('centers the manual risk workspace without rendering the driver-video panel', () => {
    ...
    expect(manualLayout).toHaveClass('roadie-navigation-density-surface')
  ...
  it('keeps one Plyr instance while driver video sources change and clear', async () => {
    ...
    expect(await screen.findByTestId('demo-scenario-presenter-panel')).toBeInTheDocument()
    expect(screen.getByTestId('driver-video-player')).toHaveAttribute('src', '/videos/drowsy-yawn.mp4?v=llast-20260715')
  ...
  it('maps the drowsy demo scenario steps to the configured driver videos', () => {
    ...
    expect(getDemoDriverVideoSource(withEvent('drowsy_session_started'))?.url).toBe('/videos/drowsy-yawn.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('drowsy_ok_response'))?.url).toBe('/videos/drowsy-normal-driving.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('drowsy_ok_acknowledged'))?.url).toBe('/videos/drowsy-normal-driving.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('drowsy_repeated_detection'))?.url).toBe('/videos/drowsy-yawn.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('drowsy_rest_area_guidance_started'))?.url).toBe('/videos/drowsy-normal-driving.mp4?v=llast-20260715')
  ...
  it('maps the phone usage demo scenario steps to the configured driver videos', () => {
    ...
    expect(getDemoDriverVideoSource({ ...state, phase: 'setup' })).toBeUndefined()
    expect(getDemoDriverVideoSource(withEvent('phone_session_started'))?.url).toBe('/videos/phone-usage-1.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('phone_first_warning'))?.url).toBe('/videos/phone-usage-1.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('phone_assist_offer'))?.url).toBe('/videos/phone-usage-1.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('phone_assist_approved'))?.url).toBe('/videos/drowsy-normal-driving.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('phone_message_preview'))?.url).toBe('/videos/drowsy-normal-driving.mp4?v=llast-20260715')
  ...
  it('maps the device operation demo scenario to device and normal-driving videos', () => {
    ...
    expect(getDemoDriverVideoSource({ ...state, phase: 'setup' })).toBeUndefined()
    expect(getDemoDriverVideoSource(withEvent('device_session_started'))?.url).toBe('/videos/device-operation.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('device_first_detection'))?.url).toBe('/videos/device-operation.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('device_first_warning'))?.url).toBe('/videos/device-operation.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('device_repeated_detection'))?.url).toBe('/videos/device-operation.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('device_music_offer'))?.url).toBe('/videos/device-operation.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('device_music_approved'))?.url).toBe('/videos/device-normal-driving.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('device_music_type_prompt'))?.url).toBe('/videos/device-normal-driving.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('device_music_started'))?.url).toBe('/videos/device-normal-driving.mp4?v=llast-20260715')
  ...
  it('maps the voice personality mini scenario to the yawn driver video for every step', () => {
    ...
    expect(getDemoDriverVideoSource(state)?.url).toBe('/videos/drowsy-yawn.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('personality_user_request'))?.url).toBe('/videos/drowsy-yawn.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('personality_clear_mode_applied'))?.url).toBe('/videos/drowsy-yawn.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('personality_clear_followup'))?.url).toBe('/videos/drowsy-yawn.mp4?v=llast-20260715')
  ...
  it('maps the gaze-away mini scenario to attention and normal-driving videos', () => {
    ...
    expect(getDemoDriverVideoSource(state)?.url).toBe('/videos/gaze-away.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('gaze_away_first_guidance'))?.url).toBe('/videos/gaze-away.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('gaze_away_user_response'))?.url).toBe('/videos/drowsy-normal-driving.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('gaze_away_resolved'))?.url).toBe('/videos/drowsy-normal-driving.mp4?v=llast-20260715')
  ...
  it('maps the reaching-behind mini scenario to rear-seat and normal-driving videos', () => {
    ...
    expect(getDemoDriverVideoSource(state)?.url).toBe('/videos/reaching-behind.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('reaching_behind_first_guidance'))?.url).toBe('/videos/reaching-behind.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('reaching_behind_user_response'))?.url).toBe('/videos/drowsy-normal-driving.mp4?v=llast-20260715')
    expect(getDemoDriverVideoSource(withEvent('reaching_behind_resolved'))?.url).toBe('/videos/drowsy-normal-driving.mp4?v=llast-20260715')
  ...
  it('keeps the rail inside the navigation viewport and connects the drawer on its right side', async () => {
    ...
    expect(railDock).toHaveClass('roadie-navigation-density-surface')
    ...
    expect(drawer).toHaveClass('roadie-navigation-density-surface')
  ...
  it('opens the report drawer and the connect drawer from the rail', async () => {
    ...
    const reportFullscreen = await screen.findByRole('dialog', { name: '전체 운행 보고서' })
    expect(reportFullscreen).toBeInTheDocument()
    expect(reportFullscreen.firstElementChild).toHaveClass('roadie-navigation-density-surface')
  ...
  it('shows route candidates before starting guidance and starts with the selected option', async () => {
    ...
    expect(routeSelectionSummary).toHaveClass('roadie-navigation-density-surface')
  ...
  it('shows a 로디 thinking modal while route candidates are loading', async () => {
    ...
    expect(loadingModal.querySelector('.roadie-assistant-aura')).toHaveClass('roadie-navigation-density-surface')
